void ReadAnalog()
{
  ...
  Analog_Z = map(Xbox.getAnalogHat(RightHatX),-32767,32767,0,1023);
  ...
  if(abs(Analog_X-Analog_X_AVG)>Lim) 
  {
    stepper.move(5*(Analog_X-Analog_X_AVG));  
  }
  else
  {
    stepper.move(0);
  ...
  { 
  stepper2.move(5*(Analog_Y-Analog_Y_AVG));  
  }
  else
  {
    stepper2.move(0);
  ...
  {
    stepper3.move(5*(Analog_Z-Analog_R_AVG));  
  }
  else
  {
    stepper3.move(0);
